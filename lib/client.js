...
Client.prototype._discover = () => {
	...
	this._soc0.on("message", (msg, rinfo) => {
...
Client.prototype._onResolved = (err, addresses) => {
	if (err) {
...
Client.prototype._onListening = () => {
	this._numSocs++;
	//console.log("this._numSocs++: " + this._numSocs);
};

Client.prototype._onClosed = () => {
	if (this._numSocs > 0) {
		this._numSocs--;
		//console.log("this._numSocs--: " + this._numSocs);
...
Client.prototype._onReceived = (msg, rinfo) => {
	let self = this,
	...
	if (this._state == State.NBDaDp) {
		...
		this._soc1.on("message", (msg, rinfo) => {
			self._onReceived(msg, rinfo);
		});
		this._soc1.on("close", () => {
			self._onClosed();
 ...
 */
Client._randTransId = () => {
	var seed = process.pid.toString(16), md5 = crypto.createHash('md5');
 ...
 */
Client.prototype.setLocalAddr = (addr) => {
 ...
 */
Client.prototype.setServerAddr = (addr, port) => {
	var d = addr.split('.');
	if (d.length != 4 || (
		isNaN(parseInt(d[0])) ||
		isNaN(parseInt(d[1])) ||
 ...
 */
Client.prototype.start = (option, cb) => {
 ...
 * are all closed.
 */
Client.prototype.close = (callback) => {
 ...
 * @type string
 */
Client.prototype.getNatType = () => {
	let natted = this.isNatted(),
		nb = this.getNB(),
		ef = this.getEF();
 ...
 * @type object
 */
Client.prototype.getMappedAddr = () => {
